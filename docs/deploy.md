...
| Sentinel | `.github/workflows/sentinel.yml`, `tools/sentinel.py` | Every hour, no credentials: rebuilds the live revision and compares every file, checks headers, redirects, 404, bucket, TLS, certificate, DNS and registration. One standing issue. |
...
| Console | `tools/console/`, with the launcher `helmet-duck-console-start.command` inside it | The owner's local, read-only web view of the live site: health, edge metrics, visitors by edge country, non-human traffic, the guard. See [console.md](console.md). |
