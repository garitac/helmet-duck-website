...
| Domain | Route 53 delegation pinned in the sentinel; CAA restricting issuance to Amazon's CAs; DMARC reject; either null MX with SPF fail-all, or WorkMail's MX, SPF, DKIM and autodiscover records once the mailbox exists; registrar transfer lock on helmetduck.com and helmet-duck.com | live since 2026-09-07; the sentinel requires them |
...
- Run `tools/mail.sh` for the WorkMail mailbox, then create the mailbox user in the
  console: it has a password, which no script here handles.
